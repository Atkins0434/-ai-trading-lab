...
NEWS_FIELDS = ('news_article_count', 'news_fetch_status', 'admitted_event_count', 'best_event_type', 'source_tier', 'verification_status', 'freshest_event_age_minutes', 'sentiment')
...
def normalize_news_snapshot(records, ticker, trading_date, freeze, envelope, policy):
    ...
    for record in records:
        ...
        event['sentiment_source'] = 'MASSIVE_INSIGHTS'
        if event['sentiment'] == 'UNKNOWN':
            event['sentiment_source'] = 'KEYWORDS'
            ...
            event['sentiment'] = 'MIXED' if negative and positive else 'NEGATIVE' if negative else 'POSITIVE' if positive else 'UNKNOWN'
