...
def test_missing_atr_keeps_fixed_denominator_and_alpha12_bytes():
    ...
    assert candidate['score_pct'] == candidate['total_score'] / 76 * 100
    ...
    original = {key: value for key, value in candidate['component_scores'].items() if key not in EXTENDED_METRIC_IDS}
...
def test_cdlx_synthetic_march15_exports_both_scores(tmp_path):
    ...
    assert candidate['rubric_version'] == 'alpha_v1.1_19m'
    ...
    with paths[0].open() as handle:
        ...
        assert len([key for key in reader.fieldnames if key.endswith('_raw')]) == 19
        row = next(reader)
    assert row['ticker'] == 'CDLX'
    assert row['rubric_version'] == 'alpha_v1.1_19m'
...
def test_registry_numbers_and_research_only_thresholds():
    ...
    assert len(METRIC_IDS) == 19
    assert registry['maximum_points'] == 76
    assert config['selection_threshold_pct'] == 70
    assert reversal['scoring']['maximum_points'] == 48


def test_postmortem_components_include_all_nineteen():
