...
### Alpha v1.3 news catalysts

The flat-file scorer requests Massive news only for tickers meeting the 10-real-bar shadow minimum, including names later rejected by a guardrail. `data/news_cache/<date>/<ticker>.json` retains raw response pages, the query window, provider/feed version, and fetch time. A successful query (including an empty result) is immutable; cache hits never query the provider again. A cache with a different freeze/window is refused as missing rather than overwritten. Fetch failures are not cached as successes and may be retried on a later run. All three replay workflows persist news alongside the reference cache.

The query window is the three days ending at freeze. Massive supplies publication time, not ingestion time: unknown availability is assumed at publication plus 15 minutes, inclusive at the admission boundary. Known provider availability remains authoritative when supplied. `news_sources.json` defines editable publisher tiers and diagnostic sentiment keywords. Only Massive ticker-specific insights supply sentiment; absent insights leave UNKNOWN. Keyword sentiment is exported as keyword_sentiment_hint for the best event and never affects scoring. Keyword rules still classify event types. News mentioning a filing is classified as OTHER; no SEC data is fetched. Admitted copies with the same headline key count once; verification considers only publishers whose copy was available by freeze.

Metrics 19–21 use the existing quality and verification point tables and the configured freshness ladder. Any admitted dilution event forces quality to zero. Freshness uses the newest relevant admitted event under the event-type rules in catalyst_policy_alpha_v1.json. Earnings, guidance, FDA/regulatory, M&A, contract, product, and SEC events are relevant regardless of sentiment; analyst, management, legal, macro, and OTHER require POSITIVE or MIXED insights sentiment. Offering/dilution is never relevant. Successful queries with no admitted events yield NO_CATALYST and zero points; failed or unavailable news yields MISSING and no points. The reachable score is now points / 100 (25 metrics); the fixed reference remains points / 120. The 70% threshold, alpha12 comparison, reversal components, and execution controls are unchanged.

`postmortem.news_coverage` counts all scored and shadow-scored rows, including rejected names. Source/event breakdowns count each ticker's best event once and match `source_tier` / `best_event_type` in the CSV; this is a ticker coverage diagnostic, not an article histogram. Selection coverage includes exploration picks, matching CSV `selected`. Price tiers use the frozen price and include UNKNOWN for missing prices. Raw article count, fetch status, admitted count, best event, verification, freshest relevant-event age, and sentiment are exported so empty queries, late news, and fetch errors can be distinguished. Cumulative PDF and workflow summaries sum these counts across completed days.

News coverage also reports shares with a relevant event strictly under 12 and 24 hours, using all scored/shadow rows as the denominator. Missing news and no relevant event remain in that denominator. Age p25/p50/p75 use only tickers with a relevant admitted event, in minutes, with linear interpolation at `(n-1)*p`; empty samples yield nulls. Retained age samples let cumulative reports pool ticker-days rather than average daily percentiles.
